...
def prediction(ensemble, db_path= 'database_images', pred_path = 'prediction_images', result_path = 'prediction_images/result_images'):
    ...
    for i in tqdm(range(len(files))):
        ...
        file = files[i]
        file_path = os.path.join(predfullpath, file)
        ...
        if ensemble:  
            m_file_path = os.path.join(resultfullpath, file[:-4]+'_pred_ensemble.jpg')
        else:
        ...
        for face in resp_obj.values():
            ...
            cv2.rectangle(image, (x, y), (x+top_result['source_w'], y+top_result['source_h']), (0, 255, 0), 2)
    ...
    print('\n[STATUS] '+str(len(files))+' images predicted successfully.')

    if ensemble:
        print('[*] Prediction outputs saved in file log_predictionoutputs_ensemble.txt')
    else:
        print('[*] Prediction outputs saved in file log_predictionoutputs.txt')

    print('[*] Prediction images saved in '+result_path+' folder.')